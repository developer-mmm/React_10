...
import { useGlobalContext } from "../../hooks/useGlobalContext";
import { auth } from "../../firebase/firebaseConfig";
import { signOut } from "firebase/auth"; // Возможно, здесь должно быть import { signOut } from "firebase/auth";

const themeFromLocalStorage = () => {
  return localStorage.getItem("theme") || "retro"; // Добавил || и исправил название функции
};
...
function Header() {
  ...
  const [theme, setTheme] = useState(themeFromLocalStorage()); // Исправил название функции
        ...
        <div className="center flex gap-10">
          ...
          <div className="indicator">
            ...
            <button className="btn uppercase -mt-2 text-xl hover:text-blue-500">
              {" "}
              Add To Cart
            </button> {/* Исправил "Add To Card" на "Add To Cart" */}
          </div>
          <div></div>
        </div>
        <div className="right">
          <label className="swap swap-rotate">
            <input
              ...
              readOnly
            />
            {/* sun icon и moon icon */}
          </label>
          <button onClick={logOut} className="btn btn-primary">Log Out</button> {/* Добавил кнопку для выхода из системы */}
        </div>
      </nav>
    </header>
  );
